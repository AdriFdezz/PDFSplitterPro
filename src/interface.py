import os
import tkinter as tk
from tkinter import ttk, filedialog, messagebox
try:
    from .logic import split_pdf
except ImportError:
    from src.logic import split_pdf

from PIL import Image, ImageTk

try:
    from tkinterdnd2 import DND_FILES, TkinterDnD

    DND_AVAILABLE = True
except ImportError:
    DND_AVAILABLE = False


class PDFSplitterApp:
    """
    Main application class for PDF Splitter Pro.
    Handles the GUI construction, event binding, and interaction logic.
    """

    def __init__(self, root):
        """
        Initialize the application.

        Args:
            root (tk.Tk or TkinterDnD.Tk): The root window of the application.
        """
        self.root = root
        self.root.title("PDF Splitter Pro")
        self.root.geometry("520x420")
        self.root.resizable(False, False)
        self.root.configure(bg="#2C2C2C")
        self.pdf_path = None

        # Set window icon
        try:
            import sys

            if getattr(sys, "frozen", False):
                icon_path = os.path.join(sys._MEIPASS, "assets", "icon.png")
            else:
                icon_path = os.path.join("assets", "icon.png")
            icon_image = Image.open(icon_path)
            icon_image = icon_image.resize((32, 32), Image.Resampling.LANCZOS)
            icon_photo = ImageTk.PhotoImage(icon_image)
            self.root.iconphoto(True, icon_photo)
            self.root.icon_photo = icon_photo
        except Exception:
            pass

        # Main frame with dark grey background
        self.frame = tk.Frame(self.root, bg="#F5F5F5", relief="flat")
        self.frame.pack(fill="both", expand=True, padx=2, pady=2)

        # Red top bar
        header = tk.Frame(self.frame, bg="#D32F2F", height=85)
        header.pack(fill="x")
        header.pack_propagate(False)

        # Title
        title_label = tk.Label(
            header,
            text="PDF Splitter Pro",
            font=("Segoe UI", 22, "bold"),
            bg="#D32F2F",
            fg="white",
        )
        title_label.pack(pady=(15, 5))

        subtitle_label = tk.Label(
            header,
            text="Professional tool for splitting PDF documents",
            font=("Segoe UI", 9),
            bg="#D32F2F",
            fg="white",
        )
        subtitle_label.pack(pady=(0, 5))

        # Main container with white background
        main_container = tk.Frame(self.frame, bg="white")
        main_container.pack(fill="both", expand=True, padx=25, pady=20)

        # Drop area with dashed border
        self.drop_canvas = tk.Canvas(
            main_container, width=450, height=100, bg="#FAFAFA", highlightthickness=0
        )
        self.drop_canvas.pack(pady=(0, 15))

        # Create dashed border
        dash_pattern = (8, 4)
        self.drop_canvas.create_rectangle(
            5, 5, 445, 95, outline="#E57373", width=2, dash=dash_pattern
        )

        # Resize app PNG icon inside the box
        try:
            import sys

            if getattr(sys, "frozen", False):
                icon_path = os.path.join(sys._MEIPASS, "assets", "icon.png")
            else:
                icon_path = os.path.join("assets", "icon.png")

            # Open PNG with full transparency
            app_icon_img = Image.open(icon_path)
            if app_icon_img.mode != "RGBA":
                app_icon_img = app_icon_img.convert("RGBA")
            app_icon_img = app_icon_img.resize((40, 40), Image.Resampling.LANCZOS)
            self.app_icon_photo = ImageTk.PhotoImage(app_icon_img)

            # Canvas without border and background matching drop area
            self.icon_canvas = tk.Canvas(
                self.drop_canvas,
                width=40,
                height=40,
                bg="#FAFAFA",
                highlightthickness=0,
                bd=0,
                relief="flat",
            )
            self.icon_canvas.place(x=15, y=30)
            self.icon_canvas.create_image(
                20, 20, image=self.app_icon_photo, anchor="center"
            )
        except Exception:
            pass

        # Drop text
        self.drop_text = self.drop_canvas.create_text(
            260,
            50,
            text="Drag your PDF file here or click to select",
            fill="#757575",
            font=("Segoe UI", 10),
        )

        # Make the entire area clickable
        self.drop_canvas.bind("<Button-1>", self.select_pdf)
        self.drop_canvas.config(cursor="hand2")

        # Configure drag and drop
        self.setup_drag_drop()

        # Slight hover effect for the drag area
        def drop_on_enter(e):
            if self.pdf_path:
                return
            self.drop_canvas.config(bg="#FFEBEE")
            if hasattr(self, "icon_canvas"):
                self.icon_canvas.config(bg="#FFEBEE")

        def drop_on_leave(e):
            if self.pdf_path:
                return
            self.drop_canvas.config(bg="#FAFAFA")
            if hasattr(self, "icon_canvas"):
                self.icon_canvas.config(bg="#FAFAFA")

        self.drop_canvas.bind("<Enter>", drop_on_enter)
        self.drop_canvas.bind("<Leave>", drop_on_leave)

        # Selected file information
        self.info_label = tk.Label(
            main_container,
            text="No file selected",
            bg="white",
            fg="#9E9E9E",
            font=("Segoe UI", 9),
        )
        self.info_label.pack(pady=(0, 15))

        # Page selection frame
        page_container = tk.Frame(main_container, bg="white")
        page_container.pack(pady=(0, 20))

        # Start page
        start_frame = tk.Frame(
            page_container, bg="#FFEBEE", relief="flat", borderwidth=0
        )
        start_frame.grid(row=0, column=0, padx=10)

        tk.Label(
            start_frame,
            text="Start Page",
            bg="#FFEBEE",
            fg="#D32F2F",
            font=("Segoe UI", 9, "bold"),
        ).pack(pady=(10, 2))
        self.start_entry = tk.Entry(
            start_frame,
            width=7,
            font=("Segoe UI", 18, "bold"),
            justify="center",
            bg="white",
            fg="#D32F2F",
            relief="flat",
            borderwidth=2,
            highlightthickness=2,
            highlightbackground="#D32F2F",
            highlightcolor="#D32F2F",
            insertbackground="#D32F2F",
        )
        self.start_entry.pack(pady=(2, 10), padx=20)

        # Arrow
        arrow_label = tk.Label(
            page_container,
            text="→",
            bg="white",
            fg="#D32F2F",
            font=("Segoe UI", 22, "bold"),
        )
        arrow_label.grid(row=0, column=1, padx=10)

        # End page
        end_frame = tk.Frame(page_container, bg="#FFEBEE", relief="flat", borderwidth=0)
        end_frame.grid(row=0, column=2, padx=10)

        tk.Label(
            end_frame,
            text="End Page",
            bg="#FFEBEE",
            fg="#D32F2F",
            font=("Segoe UI", 9, "bold"),
        ).pack(pady=(10, 2))
        self.end_entry = tk.Entry(
            end_frame,
            width=7,
            font=("Segoe UI", 18, "bold"),
            justify="center",
            bg="white",
            fg="#D32F2F",
            relief="flat",
            borderwidth=2,
            highlightthickness=2,
            highlightbackground="#D32F2F",
            highlightcolor="#D32F2F",
            insertbackground="#D32F2F",
        )
        self.end_entry.pack(pady=(2, 10), padx=20)

        # Split button
        self.split_btn = tk.Button(
            main_container,
            text="SPLIT PDF",
            command=self.split_pdf,
            font=("Segoe UI", 12, "bold"),
            bg="#D32F2F",
            fg="#FFFFFF",
            activebackground="#B71C1C",
            activeforeground="#FFFFFF",
            relief="flat",
            cursor="hand2",
            state="disabled",
            padx=50,
            pady=14,
            borderwidth=0,
            disabledforeground="#FFFFFF",
        )
        self.split_btn.pack(pady=(0, 10))

        # Hover effect for the button
        def on_enter(e):
            self.split_btn.config(bg="#B71C1C", fg="#FFFFFF")

        def on_leave(e):
            self.split_btn.config(bg="#D32F2F", fg="#FFFFFF")

        self.split_btn.bind("<Enter>", on_enter)
        self.split_btn.bind("<Leave>", on_leave)

        # Discreet help text
        help_label = tk.Label(
            main_container,
            text="Tip: Press Ctrl+V to paste the file path",
            bg="white",
            fg="#BDBDBD",
            font=("Segoe UI", 8),
        )
        help_label.pack(pady=(5, 0))

        # Keyboard shortcut
        self.root.bind("<Control-v>", self.paste_pdf)

    def setup_drag_drop(self):
        """Configure drag and drop for PDF files."""
        if DND_AVAILABLE:
            # Configure drag and drop with tkinterdnd2
            self.drop_canvas.drop_target_register(DND_FILES)
            self.drop_canvas.dnd_bind("<<Drop>>", self.on_drop)
            self.drop_canvas.dnd_bind("<<DragEnter>>", self.on_drag_enter)
            self.drop_canvas.dnd_bind("<<DragLeave>>", self.on_drag_leave)

    def on_drag_enter(self, event):
        """
        Handle drag enter event.
        Changes background color to indicate drop zone.
        """
        self.drop_canvas.config(bg="#E3F2FD")
        if hasattr(self, "icon_canvas"):
            self.icon_canvas.config(bg="#E3F2FD")

    def on_drag_leave(self, event):
        """
        Handle drag leave event.
        Restores original background color.
        """
        self.drop_canvas.config(bg="#FAFAFA")
        if hasattr(self, "icon_canvas"):
            self.icon_canvas.config(bg="#FAFAFA")

    def on_drop(self, event):
        """
        Handle drop event.
        Processes the dropped files and selects the first PDF found.
        """
        try:
            # Restore normal color
            self.drop_canvas.config(bg="#FAFAFA")
            if hasattr(self, "icon_canvas"):
                self.icon_canvas.config(bg="#FAFAFA")

            # Get dropped files
            files = self.root.tk.splitlist(event.data)

            # Find the first PDF file
            for file_path in files:
                # Clean file path
                file_path = file_path.strip("{}")
                if file_path.lower().endswith(".pdf") and os.path.isfile(file_path):
                    self.set_pdf(file_path)
                    break
        except Exception as e:
            print(f"Error in drag & drop: {e}")
            pass

    def select_pdf(self, event=None):
        """Open file dialog to select a PDF."""
        path = filedialog.askopenfilename(filetypes=[("PDF files", "*.pdf")])
        if path:
            self.set_pdf(path)

    def paste_pdf(self, event):
        """
        Handle paste event (Ctrl+V).
        Tries to get a file path from the clipboard.
        """
        # Allow pasting path from clipboard
        try:
            path = self.root.clipboard_get()
            if os.path.isfile(path) and path.lower().endswith(".pdf"):
                self.set_pdf(path)
        except:
            pass

    def set_pdf(self, path):
        """
        Load a PDF file and update the UI.

        Args:
            path (str): Path to the PDF file.
        """
        self.pdf_path = path
        filename = os.path.basename(path)

        try:
            # Get number of pages
            _, num_pages = split_pdf(path, 1, 1, os.devnull)
            self.num_pages = num_pages

            # Switch to success state
            self.drop_canvas.config(bg="#E8F5E9")
            self.icon_canvas.config(bg="#E8F5E9")

            # Change border to green
            self.drop_canvas.delete("all")
            dash_pattern = (8, 4)
            self.drop_canvas.create_rectangle(
                5, 5, 445, 95, outline="#4CAF50", width=2, dash=dash_pattern
            )

            # Change icon to checkmark
            self.icon_canvas.delete("all")

            # Green circle
            self.icon_canvas.create_oval(
                8, 8, 32, 32, fill="#4CAF50", outline="#388E3C", width=1
            )
            # Green tick
            self.icon_canvas.create_line(
                13, 20, 17, 24, fill="white", width=3, capstyle="round"
            )
            self.icon_canvas.create_line(
                17, 24, 27, 14, fill="white", width=3, capstyle="round"
            )

            # X button to remove file (top right corner of canvas)
            self.clear_btn_canvas = tk.Canvas(
                self.drop_canvas,
                width=20,
                height=20,
                bg="#E8F5E9",
                highlightthickness=0,
                bd=0,
                relief="flat",
            )
            self.clear_btn_canvas.place(x=420, y=10)

            # Only red circle for X
            self.clear_btn_canvas.create_oval(
                2, 2, 18, 18, fill="#F44336", outline="#D32F2F", width=1
            )
            # White X
            self.clear_btn_canvas.create_line(
                6, 6, 14, 14, fill="white", width=2, capstyle="round"
            )
            self.clear_btn_canvas.create_line(
                14, 6, 6, 14, fill="white", width=2, capstyle="round"
            )
            # Make clickable + hover effect
            self.clear_btn_canvas.bind("<Button-1>", self.clear_pdf)
            self.clear_btn_canvas.config(cursor="hand2")

            def clear_on_enter(e):
                self.clear_btn_canvas.config(bg="#E8F5E9")
                self.clear_btn_canvas.delete("all")
                self.clear_btn_canvas.create_oval(
                    2, 2, 18, 18, fill="#D32F2F", outline="#B71C1C", width=1
                )
                self.clear_btn_canvas.create_line(
                    6, 6, 14, 14, fill="white", width=2, capstyle="round"
                )
                self.clear_btn_canvas.create_line(
                    14, 6, 6, 14, fill="white", width=2, capstyle="round"
                )

            def clear_on_leave(e):
                self.clear_btn_canvas.config(bg="#E8F5E9")
                self.clear_btn_canvas.delete("all")
                self.clear_btn_canvas.create_oval(
                    2, 2, 18, 18, fill="#F44336", outline="#D32F2F", width=1
                )
                self.clear_btn_canvas.create_line(
                    6, 6, 14, 14, fill="white", width=2, capstyle="round"
                )
                self.clear_btn_canvas.create_line(
                    14, 6, 6, 14, fill="white", width=2, capstyle="round"
                )

            self.clear_btn_canvas.bind("<Enter>", clear_on_enter)
            self.clear_btn_canvas.bind("<Leave>", clear_on_leave)

            # Update text on canvas
            if len(filename) > 40:
                filename = filename[:37] + "..."
            self.drop_canvas.create_text(
                260,
                50,
                text=f"File loaded: {filename}\n{num_pages} page{'s' if num_pages > 1 else ''}",
                fill="#2E7D32",
                font=("Segoe UI", 10, "bold"),
                justify="center",
            )

            # Update information
            self.info_label.config(
                text=f"✓ Ready to split ({num_pages} pages)",
                fg="#4CAF50",
                font=("Segoe UI", 9, "bold"),
            )

            # Enable button
            self.split_btn.config(state="normal", bg="#D32F2F")

            # Fill fields
            self.start_entry.delete(0, tk.END)
            self.end_entry.delete(0, tk.END)
            self.start_entry.insert(0, "1")
            self.end_entry.insert(0, str(self.num_pages))

        except Exception as e:
            messagebox.showerror("Error", f"Could not read PDF:\n{e}")

            # Elegant error state
            self.drop_canvas.config(bg="#FFEBEE")
            self.icon_canvas.config(bg="#FFEBEE")

            # Change border to red
            self.drop_canvas.delete("all")
            dash_pattern = (8, 4)
            self.drop_canvas.create_rectangle(
                5, 5, 445, 95, outline="#F44336", width=2, dash=dash_pattern
            )

            # Error icon
            self.icon_canvas.delete("all")
            self.icon_canvas.create_oval(
                12, 12, 38, 38, fill="#F44336", outline="#D32F2F", width=2
            )
            self.icon_canvas.create_text(
                25, 25, text="✗", fill="white", font=("Segoe UI", 16, "bold")
            )

            # Error text
            self.drop_canvas.create_text(
                260,
                50,
                text="Error loading file\nTry another PDF",
                fill="#C62828",
                font=("Segoe UI", 10),
                justify="center",
            )

            self.info_label.config(
                text="✗ Error loading file", fg="#F44336", font=("Segoe UI", 9, "bold")
            )

    def clear_pdf(self, event=None):
        """Clear loaded file and restore initial state."""
        self.pdf_path = None
        self.num_pages = None

        # Restore initial canvas state
        self.drop_canvas.config(bg="#FAFAFA")
        if hasattr(self, "icon_canvas"):
            self.icon_canvas.config(bg="#FAFAFA")

        # Clear canvas and restore initial elements
        self.drop_canvas.delete("all")

        # Recreate original dashed border
        dash_pattern = (8, 4)
        self.drop_canvas.create_rectangle(
            5, 5, 445, 95, outline="#E57373", width=2, dash=dash_pattern
        )

        # Restore original app icon
        if hasattr(self, "app_icon_photo"):
            self.icon_canvas.delete("all")
            self.icon_canvas.create_image(
                20, 20, image=self.app_icon_photo, anchor="center"
            )

        # Restore original text
        self.drop_text = self.drop_canvas.create_text(
            260,
            50,
            text="Drag your PDF file here or click to select",
            fill="#757575",
            font=("Segoe UI", 10),
        )

        # Remove X button if exists
        if hasattr(self, "clear_btn_canvas"):
            self.clear_btn_canvas.destroy()
            delattr(self, "clear_btn_canvas")

        # Restore info state
        self.info_label.config(
            text="No file selected", fg="#9E9E9E", font=("Segoe UI", 9)
        )

        # Disable split button
        self.split_btn.config(state="disabled")

        # Clear entry fields
        self.start_entry.delete(0, tk.END)
        self.end_entry.delete(0, tk.END)

    def split_pdf(self):
        """Execute the PDF split operation based on user input."""
        if not self.pdf_path:
            return
        try:
            start = int(self.start_entry.get())
            end = int(self.end_entry.get())
            if start < 1 or end > self.num_pages or start > end:
                messagebox.showerror("Error", "Invalid page range.")
                return
            out_path = filedialog.asksaveasfilename(
                defaultextension=".pdf",
                filetypes=[("PDF files", "*.pdf")],
                initialfile=f"split_{start}-{end}.pdf",
            )
            if out_path:
                split_pdf(self.pdf_path, start, end, out_path)
                messagebox.showinfo("Success", f"PDF saved to {out_path}")
        except Exception as e:
            messagebox.showerror("Error", f"Could not split PDF: {e}")
